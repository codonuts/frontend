...
class _LogsScreenState extends State<LogsScreen> {
  ...
  void _openReport(InspectionItem item) {
    List<String> docs = [];
    if (item.pdfPath != null) docs.add(item.pdfPath!);
    if (item.isViolation) docs.add('Statutory_Notice_Form_III.pdf');
    docs.add('Inspection_Photographs.zip');
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => CaseDetailsScreen(
          ...
          directive: item.isViolation ? 'Violation Detected under Rule 6/9 PCR 2011' : 'Compliant with Package Commodities Rules',
          date: item.formattedTime,
          location: 'North District Commercial Zone',
          evidence: docs,
          status: item.isViolation ? 'Violation' : 'Compliant',
        ),
      ),
    );
  }
